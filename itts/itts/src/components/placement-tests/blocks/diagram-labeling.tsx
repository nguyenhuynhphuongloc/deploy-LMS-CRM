        ...
        <div className="rounded-lg bg-[#E7EAE9] px-6 py-4">
          {matchingLabel ? (
            <h3 className="mb-3 text-[16px] font-bold text-[#151515]">
              <TextHighlighter cacheKey={`${cacheKey}-label`}>
                {matchingLabel}
              </TextHighlighter>
            </h3>
          ) : null}
            ...
            {(Array.isArray(options) ? options : [])?.map(
              (option: BlockOptionType) => (
                <p key={option.id} className="text-[14px]">
                  ...
                  <TextHighlighter cacheKey={`${cacheKey}-opt-${option.id}`}>
                    {option.label}
                  </TextHighlighter>
                </p>
              ),
                      ...
                      {(Array.isArray(options) ? options : [])?.map(
                        (option: BlockOptionType) => (
                          <SelectItem key={option.id} value={option.value}>
                            ...
                            <TextHighlighter
                              cacheKey={`${cacheKey}-q-${question.id}-opt-${option.id}`}
                            >
                              {option.label}
                            </TextHighlighter>
                          </SelectItem>
