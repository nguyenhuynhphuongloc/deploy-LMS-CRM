...
function useCollabAuthorName(): string {
  ...
  return yjsDocMap.has("comments") ? name : "User";
}
...
function CommentPlugin({
  ...
  const commentStore = useMemo(() => {
    const store = new CommentStore(editor);
    if (initialComments && initialComments.length > 0) {
      (store as any)._comments = initialComments;
    }
    return store;
...
export default React.memo(CommentPlugin, () => true);
