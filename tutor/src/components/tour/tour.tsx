...
import { HAS_SEEN_TOUR_KEY } from "@/constants";
...
export function TourProvider({
  ...
  const [storageKey, setStorageKey] = useState(HAS_SEEN_TOUR_KEY);

  useEffect(() => {
    if (typeof window !== "undefined") {
      const hasSeen = localStorage.getItem(storageKey) === "true";
      setIsCompleted(hasSeen);
  ...
  const updateElementPosition = useCallback(() => {
    if (currentStep >= 0 && currentStep < steps.length) {
      ...
      const isFixed = steps[currentStep]?.isFixedElement ?? false;
      const position = getElementPosition(selectorId);

      if (!element || !position) {
        // Retry once after a short delay
        setTimeout(() => {
          const retryElement = document.getElementById(selectorId);
          const retryPosition = getElementPosition(selectorId);
          if (retryElement && retryPosition) {
            if (!isFixed) {
      ...
      if (!isFixed) {
        ...
        const newPosition = getElementPosition(selectorId);
        if (newPosition) {
  ...
  }, [steps.length, onComplete, currentStep, storageKey, setIsTourCompleted]);
      ...
      <AlertDialogContent className="max-w-[500px] p-6">
        <AlertDialogHeader className="flex flex-col items-center justify-center">
          <div className="relative mb-4">
            ...
            >
              <Image src="/mascot_hi.png" width={128} height={128} alt="Logo" />
            </motion.div>
          </div>
          <AlertDialogTitle className="text-center text-[22px] font-semibold">
            {title}
          </AlertDialogTitle>
          <AlertDialogDescription className="mt-2 text-[16px]">
            {description}
          </AlertDialogDescription>
        </AlertDialogHeader>
